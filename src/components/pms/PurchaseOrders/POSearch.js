          ...
          <div style={{ textAlign: 'center' }}>
            <i
              ...
              onClick={() => {
                ...
                API.get(
                  `/pms/purchaseOrder/get/line/${props.original.PO_Header_ID}`
                )
                  .then(response => {
                    console.log(response);
                    dispatch(fill_table(response.data));
                    window.open(
                      `/purchaseOrder/true/${props.original.PO_Header_ID}`,
                      '_blank'
                    );
                  })
                  .catch(err => {
      ...
      Cell: props => {
        return (
          <div style={{ textAlign: 'center' }}>
            <Link
              style={{ color: 'black' }}
              className={props.original.Status !== 'y' && 'disabled-link'}
              target='_blank'
              to={`/report?title=PurchaseOrder&Header_ID=${
                props.original.PO_Header_ID
              }&Invoice_Number=${props.original.PO_NO}&Creation_Date=${moment(
                props.original.PO_Date
              ).format('DD-MM-YYYY')}&Payment_Type=${
                props.original.Payment_Type
              }&Supplier=${props.original.Supplier_Name}&Delivery=${
                props.original.Department_Name
              }&RefNo=${props.original.Ref_No}&Status=${
                props.original.Status
              }&Flag=${props.original.Enabled_Flag}`}
            >
              <i
                className={
                  props.original.Status !== 'y'
                    ? 'fas fa-file-excel disabled fa-2x'
                    : 'fas fa-file-invoice fa-2x'
                }
              ></i>
            </Link>
          </div>
