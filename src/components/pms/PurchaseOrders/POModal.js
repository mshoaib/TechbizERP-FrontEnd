...
const PurchaseInsertModal = props => {
  ...
  useEffect(() => {
    /* Intialize the value for AutoComplete InputBox*/
  ...
  const onSubmit = data => {
    ...
    if (props.type === 'update') {
      ...
      API.get(
        `/pms/purchaseOrder/item/get/item-details?Item_ID=${Item_ID}&UOM_ID=${UOM_ID}`
      )
        .then(response => {
      ...
      <Grid container justify='center'>
        <form onSubmit={handleSubmit(onSubmit)} style={{ minWidth: '100%' }}>
          ...
          <Autocomplete
            ...
            defaultValue={
              props.type === 'update'
                ? items.find(item => item.Item_ID === props.record.Item_ID)
                : {}
            }
          ...
          <TextField
            ...
            name='Price'
            style={{ marginBottom: '5px' }}
            defaultValue={props.type === 'update' ? props.record.Price : ''}
          ...
          <TextField
            ...
            name='Item_Qty'
            style={{ marginBottom: '5px' }}
            defaultValue={props.type === 'update' ? props.record.Item_Qty : ''}
            inputRef={register({ required: true })}
            label='Item Quantity'
          />
          ...
          <TextField
            ...
            name='GST_Per'
            style={{ marginBottom: '5px' }}
            defaultValue={props.type === 'update' ? props.record.GST_Per : ''}
            inputRef={register({ required: true })}
