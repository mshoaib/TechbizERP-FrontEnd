...
const PurchaseOrderHeader = props => {
  ...
  const [header, setHeader] = useState(() =>
    props.edit === 'true' ? props.data : initialState
  );
  ...
  useEffect(() => {
    // whenever there is change in header update HeaderInfo in POForm
  ...
  useEffect(() => {
    ...
    axios
      .all([
        API.get(`/pms/purchaseOrder/supplier/1`),
        API.get('/pms/purchaseOrder/department/1')
      ])
      .then(
        axios.spread((url1, url2) => {
  ...
  return (
    ready && (
      <div>
        <label style={{ fontSize: '24px', display: 'block' }}>
          PURCHASE ORDER
        </label>
              ...
              <Autocomplete
                ...
                defaultValue={
                  props.edit
                    ? suppliers.find(
                        supplier =>
                          supplier.Supplier_Name === props.data.Supplier_Name
                      )
                    : {}
                }
                onChange={(event, values) =>
                  setHeader({
                    ...header,
                    Supplier_ID: values && values.Supplier_ID
                  })
                }
                style={{ marginBottom: '5px', width: '70%', margin: '0 15%' }}
                renderInput={params => (
            ...
            <Grid item xs={6}>
              <Autocomplete
                ...
                defaultValue={
                  props.edit
                    ? PaymentType.find(
                        item => item.title === props.data.Payment_Type
                      )
                    : {}
                }
            ...
            <Grid item xs={6}>
              <Autocomplete
                ...
                onChange={(event, values) =>
                  setHeader({
                    ...header,
                    Ship_To_ID: values && values.Department_ID
                  })
                }
                style={{ marginBottom: '5px', width: '70%', margin: '0 15%' }}
                renderInput={params => (
